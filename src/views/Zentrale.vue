...
const pendingTimers = ref({}) // Timer für Countdown-Abbruch
...
function triggerAction(item, event) {
  ...
  // ABBRUCH LOGIK: Wenn Timer bereits läuft, lösche ihn
  if (pendingTimers.value[id]) {
  ...
  pendingTimers.value[id] = setTimeout(async () => {
    const token = localStorage.getItem('workly_token')
  ...
  <div class="zentrale-container">
    ...
    <div class="content-card">
      <BaseTable 
        ...
        :columns="columns" :data="notifications" @row-click="(i) => { selectedNotification=i; detailModal.open(); }"
      >
        <template #col-text="{ value, item }">
          <span :class="{ 'is-pending': pendingTimers[item.id] }">{{ value }}</span>
        </template>

        <template #col-actions="{ item }">
          <button 
            class="countdown-btn" 
            :class="{ 'wait-mode': pendingTimers[item.id] }"
            @click.stop="triggerAction(item, $event)"
          >
            <span v-if="pendingTimers[item.id]">Abbrechen</span>
            <span v-else>Erledigen</span>
            <div v-if="pendingTimers[item.id]" class="progress-bar"></div>
          </button>
        </template>
      </BaseTable>
...
.progress-bar { position: absolute; bottom: 0; left: 0; height: 3px; background: #ef4444; width: 100%; animation: shrink 5s linear forwards; }
